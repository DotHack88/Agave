...
<div class="grid-2">
  <div class="card card-inbound-dark">
...
<div class="grid-2">
  ...
  renderMovements() {
    ...
    // Apply sorting to movements list (collapsing double spaces and ignoring for comparison)
    if (filt.sortKey) {
      moves.sort((a, b) => {
        ...
        if (typeof valA === 'string') {
          ...
          const cleanB = String(valB).toLowerCase().replace(/\s+/g, '');
          return filt.sortOrder === 'asc' ? cleanA.localeCompare(cleanB) : cleanB.localeCompare(cleanA);
        } else {
