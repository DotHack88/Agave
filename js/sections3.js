...
Object.assign(Sections, {
  ...
  renderCSV() {
    ...
    el.innerHTML = `
<div class="page-header"><h1>Importazione CSV</h1>
  <div class="actions">
...
<div class="card">
  ...
  <div class="dropzone" id="csv-dropzone" onclick="document.getElementById('csv-file-input').click()"
    ...
    <input type="file" id="csv-file-input" accept=".csv,.txt" onchange="Sections._handleCSVFile(this)"/>
  </div>
  <div style="margin-top:16px">
    <label class="form-group"><label>Separatore colonne</label>
      <select id="csv-delimiter"><option value=",">Virgola (,)</option><option value=";">Punto e virgola (;)</option><option value="\t">Tab</option></select>
    </label>
  </div>
</div>
<div class="card">
  ...
  <div style="font-size:.83rem;line-height:2">
    <table style="width:100%"><thead><tr><th>Colonna CSV</th><th>Campo</th><th>Obbligatorio</th></tr></thead><tbody>
    ${[['codice / product_code','Codice prodotto','No'],['barcode / ean','Barcode EAN','No'],
      ['nome / name / prodotto','Nome prodotto','✅ Sì'],['categoria / category','Categoria','No'],
      ['marca / brand','Marca','No'],['quantita / quantity / qty','Quantità','No'],
      ['quantita_minima / qty_min','Scorta minima','No'],['prezzo_acquisto / costo','Prezzo acquisto','No'],
      ['prezzo_vendita / prezzo','Prezzo vendita','No'],['fornitore / supplier','Fornitore','No'],
      ['ubicazione / location','Ubicazione','No']].map(([col,field,req])=>
      `<tr><td class="td-code" style="font-size:.78rem">${col}</td><td>${field}</td><td style="color:${req.includes('Sì')?'var(--green)':'var(--text2)'}">${req}</td></tr>`).join('')}
    </tbody></table>
  </div>
</div>
</div>
  ...
  _importCSV(rows) {
    ...
    let html = `<div class="card" style="margin-top:16px">
      ...
      <div class="stats-grid" style="margin-bottom:${results.errors.length?'16px':'0'}">
        <div class="stat-card"><div class="stat-icon green"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg></div>
          <div class="stat-info"><div class="stat-value">${results.created}</div><div class="stat-label">Nuovi prodotti</div></div></div>
        <div class="stat-card"><div class="stat-icon blue"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/></svg></div>
          <div class="stat-info"><div class="stat-value">${results.updated}</div><div class="stat-label">Aggiornati</div></div></div>
        <div class="stat-card"><div class="stat-icon red"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div>
          <div class="stat-info"><div class="stat-value">${results.errors.length}</div><div class="stat-label">Errori</div></div></div>
      </div>`;
    if (results.errors.length) {
      html += `<div style="margin-top:8px"><h4 style="margin-bottom:8px;color:var(--red)">Errori rilevati:</h4>
        ${results.errors.slice(0,10).map(e=>`<div style="background:var(--red-soft);border-radius:var(--radius-sm);padding:8px 12px;margin-bottom:6px;font-size:.82rem">
          <b>Riga ${e.row}:</b> ${e.errors.join(', ')}</div>`).join('')}</div>`;
    }
    html += '</div>';
    area.innerHTML = html;
...
<div>
  <div class="card" style="margin-bottom:16px">
    ...
    <div class="settings-section">
      ...
      <div class="settings-row">
        <div><div class="settings-label">Notifiche scorte basse</div><div class="settings-sub">Mostra avvisi per prodotti sotto scorta</div></div>
